...
class BrowserHistory extends Component {
  ...
  render() {
    ...
    const newSearchResults = searchResults.filter(eachResult =>
      eachResult.title.toLowerCase().includes(searchInput.toLowerCase()),
    )

    return (
      <div className="browser-history-app-conatiner">
        ...
        {newSearchResults.length > 0 ? (
          <div className="history-list-container">
            <ul className="history-list">
              {newSearchResults.map(eachBrowserItem => (
                <BrowserItem
                  browserItemDetails={eachBrowserItem}
                  onClickDeleteIcon={this.onClickDeleteIcon}
                  key={eachBrowserItem.id}
                />
              ))}
            </ul>
          </div>
